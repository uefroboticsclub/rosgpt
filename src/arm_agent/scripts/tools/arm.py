...
def init_publishers():
    ...
    # For Interbotix PX100 in simulation
    controller_publishers['arm'] = rospy.Publisher(
        '/px100/arm_controller/command',
        JointTrajectory,
        queue_size=10
    )
    controller_publishers['gripper'] = rospy.Publisher(
        '/px100/gripper_controller/command',
        JointTrajectory,
        queue_size=10
    )
    rospy.loginfo("ARM AGENT: Publishers initialized for PincherX-100 simulation")

@tool
def get_joint_states() -> Dict[str, float]:
    """Get the current joint states of the PincherX-100 arm."""
    try:
        # Try to get joint states from the arm controller state
        try:
            msg = rospy.wait_for_message(
                '/px100/arm_controller/state',
            ...
            for i, name in enumerate(msg.joint_names):
                if i < len(msg.actual.positions):
                    joint_positions[name] = msg.actual.positions[i]
            return joint_positions
        except:
            # Fallback to general joint states
            msg = rospy.wait_for_message('/joint_states', JointState, timeout=5)
            joint_positions = {}
            for i, name in enumerate(msg.name):
                if i < len(msg.position):
...
def move_to_home_position() -> str:
    ...
    return execute_joint_trajectory(joint_positions, 3.0)
...
def execute_joint_trajectory(
    ...
    try:
        ...
        # Publish the trajectory
        controller_publishers['arm'].publish(trajectory)
        
        # Wait for movement to complete
        rospy.sleep(duration + 0.5)
        
        # Get final position
        final_states = get_joint_states()
...
@tool
def control_gripper(position: float) -> str:
    """Control the gripper opening (0.0 = closed, 1.0 = open)."""
    global controller_publishers
    ...
    try:
        # Create trajectory for gripper
        trajectory = JointTrajectory()
        trajectory.joint_names = ['left_finger', 'right_finger']
        
        point = JointTrajectoryPoint()
        # Convert 0-1 range to actual gripper joint values
        gripper_value = position * 0.037  # Max opening is 0.037
        point.positions = [gripper_value, gripper_value]
        point.time_from_start = rospy.Duration(1.0)
        
        trajectory.points = [point]
        
        controller_publishers['gripper'].publish(trajectory)
        rospy.sleep(1.0)
        
        return f"Gripper moved to position: {position} ({'closed' if position < 0.1 else 'open'})"
...
def open_gripper() -> str:
    """Open the gripper fully."""
    return control_gripper(1.0)

@tool
def close_gripper() -> str:
    """Close the gripper fully."""
    return control_gripper(0.0)
...
def get_workspace_info() -> dict:
    ...
    return {
        ...
        "available_joints": ["waist", "shoulder", "elbow", "wrist_angle", "wrist_rotate", "left_finger", "right_finger"]
    }