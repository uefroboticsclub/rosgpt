...
class ROSGPT:
    ...
    async def astream(self, query: str) -> AsyncIterable[Dict[str, Any]]:
        ...
        Raises:
            ValueError: If streaming is not enabled for this ROSGPT instance.
        ...
        if not self.__streaming:
            raise ValueError(
                "Streaming is not enabled. Use 'invoke' method instead or initialize ROSGPT with streaming=True."
            )
    ...
    def _get_tools(
        ...
        """Create a ROSGPT tools object with the specified ROS version, tools, packages, and blacklist."""
        rosgpt_tools = ROSGPTTools(ros_version, blacklist=blacklist)
        if tools:
            rosgpt_tools.add_tools(tools)
        if packages:
            rosgpt_tools.add_packages(packages, blacklist=blacklist)
        return rosgpt_tools
